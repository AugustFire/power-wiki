...
- `/` (HomeView,team 空间渲染 page tree;personal 空间渲染 MeDashboardView) / `/me` (MeDashboardView,M2 「Your Work」个人 Dashboard) / `/p/:id` (ReadView) / `/p/:id/edit` (EditView) / `/new?parent=:id` (EditView) — 需登录
...
| `DateInline` (`dateInlineExtension.ts` + `DateInlineView.vue` + `DateTimePicker.vue`) | 行内日期 atom 节点,钉死一个 ISO 日期(YYYY/MM/DD);点击节点弹 DateTimePicker 改日期 |
...
## Modal / Drawer / ConfirmDialog chrome

所有居中弹窗走 `apps/web/src/components/ui/Modal.vue`,右侧抽屉走 `Drawer.vue`,确认弹窗走 `ConfirmDialog.vue`。三组件共用同一套 chrome:

- **body 滚动锁** — 打开时存 `prevBodyOverflow = document.body.style.overflow`,设 `hidden`;关闭恢复。`onBeforeUnmount` 兜底。
- **ESC 关闭** — 绑 `document` 的 `keydown`,避免被嵌套子组件 stopPropagation 吞掉。
- **focus trap** — `apps/web/src/composables/useFocusTrap.ts`(vanilla composable,零三方依赖)。打开时记 `returnFocus = document.activeElement`,下一帧聚焦首 focusable(或 `opts.initialFocus` 指定的 CSS selector / Ref);Tab / Shift+Tab 在边界 wrap;关闭时 `returnFocus.focus()` 恢复。**keydown listener 绑到 container 而非 document**,避免跟组件既有的 ESC 监听重复触发。
- **焦点环硬约束** — 项目全局 `:focus-visible` 在 `components.css:183-191` 只对键盘聚焦显 `--focus-ring`。**编程式 `.focus()` 浏览器 heuristic 不触发 `:focus-visible`**,鼠标点开 dialog 不闪聚焦框;键盘打开才显环。**不要**手动加 `outline`,这条契约不要破坏。

`ConfirmDialog` 用 `initialFocus: '[autofocus]'`,让主按钮(cancel / confirm 中的 confirm)拿到初始焦点,Enter 直接提交。

`Drawer` **只 trap 面板不 trap backdrop** —— 焦点在 panel 内循环,Esc 关闭整个 drawer;不要把 backdrop 放进 container(否则会 trap 整个文档)。

`EmptyState` 组件(`apps/web/src/components/ui/EmptyState.vue`,props `{title, icon?, hint?, variant?, size?}` + slots `{icon, hint, default}`)是 view 空态的标准 UI,管理后台 People / Spaces / Trash 的空态都走它,不要 view 内自绘 `<div class="empty">`。
