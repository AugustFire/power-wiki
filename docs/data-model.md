# Data Model

Drizzle schema、recursive CTE 模式、auth 设计、空间隔离、级联删除约定、附件元数据、评论 / 通知 / 标签表。

## Schema 概览

`apps/api/src/db/schema.ts` **21 张表**,**无外键约束**(所有表不写 `.references()`,所有 `ALTER TABLE ... ADD CONSTRAINT FOREIGN KEY` 类的 DDL 都不写)。级联删除必须显式在路由里用 recursive CTE + 多步事务完成。migration 历史见 `apps/api/src/db/migrations/0000..0029`(最近 6 条 `0024..0029` 是 Phase A/B/C/D + 审计 kind 扩展)。

| 表 | 用途 | 引入 |
|---|---|---|
| `users` | 用户账号,argon2 哈希 + 状态 + 时间戳 + M11 avatar 三态 | 0.4 + 0022 |
| `sessions` | DB sessions,HTTP-only cookie | 0.4 |
| `user_avatars` | M11 用户自定义上传头像(`userId` + `bucketKey` + MIME + 尺寸),公开 raw 流式 | 0022 |
| `user_groups` | 用户组 | 0.4 |
| `user_group_members` | 多对多 | 0.4 |
| `spaces` | 空间(`personal` / `shared`) | 0.5 |
| `space_group_access` | **legacy** 空间对哪些组可见(Phase A.5 起写入迁移到 `space_role_grants`) | 0.5 |
| `space_role_grants` | Phase A 空间角色授权(`principalKind=user\|group`,`role=viewer\|editor\|admin`) | 0024 |
| `pages` | 页面主表(`contentJSON` + `contentHTML` 双写) | 0.4 |
| `page_versions` | 版本历史(retention 30 行,边界快照 + restore) | 0.4 + 0.8 |
| `page_labels` | 页面标签(全局自由文本,Notion 风格,非 Confluence 命名空间) | 0.8 |
| `page_restrictions` | Phase B 页面级 view / edit 限制,反 default-deny | 0026 |
| `page_public_shares` | Phase D 公开链接(sha256 tokenHash,明文永不入库) | 0028 |
| `comments` | 评论 + 二级回复(顶 + replies) | 0.6 |
| `notifications` | 通知(mention / reply / comment_on_my_page / page_like / M13 watch fanout 6 类) | 0.6 + 0.10 |
| `attachments` | 页面级附件元数据,S3 object 字节在 MinIO | 0.9 |
| `page_likes` | 页面点赞(顶栏 👍),复合 PK `(page_id, user_id)` | 0.10 |
| `admin_settings` | admin 后台全局键值设置(按 `key` 读 / 写;0.10 引入) | 0.10 |
| `permission_audit` | Phase C append-only 权限变更审计日志(11 kind × 5 target_kind) | 0027 + 0029 |
| `page_events` | 页面事件轨迹(`created` / `edited` / `moved` / `restored` / `duplicated` / `published` / `trashed` / `purged`,8 种),ActivityView 数据源 | 0.10 |
| `user_watched_pages` | per-user 关注页(顶栏 watch toggle),复合 PK `(user_id, page_id)` | 0.10 |
| `user_recent_pages` | per-user 最近浏览(`(user_id, page_id)` 复合 PK,跨设备同步,替代 localStorage) | M2 |

## 关键表字段

### `pages`

```ts
{
  id: string;              // nanoid(10),31 字符字母表(去掉 0/o/1/i/l)
  parentId: string | null; // null = 顶级
  spaceId: string;         // 必填,创建时校验;nullable at DB 层(migration safety)
  title: string;           // default DEFAULT_TITLE(共享常量)
  contentJson: jsonb;      // Tiptap JSON(default '{}')
  contentHtml: text;       // Tiptap HTML(ReadView 渲染,sanitize 后)
  icon: string | null;
  sortOrder: number;       // 同级排序,lower comes first
  createdAt: bigint;       // Date.now() ms
  updatedAt: bigint;
  authorId: string;        // free-form,可为 'me'(旧 seed)
  deletedAt: bigint | null;// 软删除时间,NULL = 正常
  deletedBy: string | null;
}
```

`contentJSON` + `contentHTML` 双写 —— JSON 给编辑器回填(避免重新解析 HTML),HTML 给 ReadView 渲染(避免 JSON → HTML 的开销 + 序列化风险)。

### `page_versions`

```ts
{
  id: string;
  pageId: string;          // No FK
  versionNumber: number;   // monotonic per-page(MAX+1 事务内算)
  title: string;           // editable fields 快照
  contentJson: jsonb;
  contentHtml: text;
  icon: string | null;
  editedBy: string;        // free-form(同 pages.authorId)
  editedAt: bigint;        // Date.now() ms
  changeNote: string | null; // restore 路径自动设 "restored from v{N}"
}
```

索引:`page_versions_page_idx(pageId, versionNumber)`(list 路径)+ `page_versions_page_version_uq(pageId, versionNumber)`(并发 PATCH 的并发保护)。**metadata-only PATCH 不写这一行**(labels / watch toggle 等)。

### `page_labels`

```ts
{
  pageId: string;          // No FK
  label: string;           // lowercase + trim + ≤32 字符,服务端 normalize
  authorId: string;        // free-form
  createdAt: bigint;
}
```

**复合 PK = (pageId, label)** —— 同 label 重复加 idempotent。索引 `page_labels_label_idx`(`/api/labels/search` 走这里)。**作用域全局**:同一个 `工程` label 可挂在多个 team space 的页面上,搜索 JOIN 用户可见 pages 过滤;不像 Confluence 有 namespace 隔离。

### `comments`

```ts
{
  id: string;
  pageId: string;          // No FK
  parentId: string | null; // null = 顶,replies 只挂顶,**v0 最大嵌套深度 = 2**
  authorId: string;        // free-form
  contentMd: string;       // markdown body
  contentText: string;     // markdown-stripped 快照(通知预览直接用)
  mentionedUserIds: jsonb; // text[],通知 fan-out 唯一来源,last-write-wins(v0 接受)
  isEdited: boolean;       // PATCH 后 true
  editedAt: bigint | null;
  createdAt: bigint;
  updatedAt: bigint;
  deletedAt: bigint | null;// 软删除(author 或 admin),deletedBy 记录谁
  deletedBy: string | null;
}
```

索引 4 条:`comments_page_idx(pageId, createdAt)` / `comments_parent_idx(parentId)` / `comments_author_idx(authorId)` / `comments_page_live_idx(pageId, deletedAt)`(list 路径过滤 live 行)。

### `notifications`

```ts
{
  id: string;
  userId: string;          // 收件人,**私域**,不暴露给别人
  actorId: string;         // 触发人(LEFT JOIN users 拿 actorName)
  kind: 'mention' | 'reply' | 'comment_on_my_page' | 'page_like';
  pageId: string;          // 落地目标,/p/{pageId}#comment-{commentId}
  pageTitle: string | null;// 触发时的标题快照(后续页面改名不追溯)
  commentId: string | null;
  mentionUserId: string | null; // 仅 kind='mention' 有
  isRead: boolean;        // default false
  readAt: bigint | null;
  createdAt: bigint;
}
```

索引 3 条:`notifications_user_unread_idx(userId, isRead, createdAt)`(bell 红点 hot path)+ `notifications_user_created_idx(userId, createdAt)`(列表 hot path)+ `notifications_page_idx(pageId)`(page purge cascade)。**只看自己的**(`WHERE user_id=me.id`,无 admin bypass)。

### `attachments`

```ts
{
  id: string;              // nanoid(12),也是 S3 object key 第二段
  pageId: string;          // No FK
  uploaderId: string;      // No FK
  originalFilename: string; // ≤255 字符,展示用
  storageKey: string;      // 格式 {page_id}/{id}{ext}
  mimeType: string;        // ALLOWED_MIME_TYPES 之一
  sizeBytes: number;       // HeadObject 写入,不信前端 size
  kind: 'image' | 'file';  // 决定编辑器 / read view 渲染 image 节点还是 file 卡片
  createdAt: bigint;       // Date.now() ms
}
```

**字节存 MinIO,DB 只存元数据**。S3 object 删除是 best-effort(DB 是事实来源,orphan 对象容忍)。索引 2 条:`attachments_page_idx(pageId, createdAt)` + `attachments_uploader_idx(uploaderId)`。

### `user_recent_pages`(per-user 最近浏览)

```ts
{
  userId: string;          // No FK
  pageId: string;          // No FK
  visitedAt: bigint;
  title: string;           // 冗余存 title,删页时历史仍能显示
}
```

**复合 PK `(user_id, page_id)`**,ReadView mount 时 upsert(`visited_at = now`)。Page delete 同步清掉对应行 —— 历史不留孤儿。取代 v0 的 localStorage `power-wiki:recents` 键,跨设备同步。索引 1 条:`user_recent_pages_user_visited_idx(userId, visitedAt)`(列表路径)。

### `user_avatars`(M11,用户自定义头像)

```ts
{
  id: string;              // nanoid(12),与 attachments 同款,也是 S3 object key 第二段
  userId: string;          // No FK
  bucketKey: string;       // 完整 bucket key,格式 `users/{userId}/{id}{ext}`
  mime: string;            // 限 AVATAR_ALLOWED_MIME(png/jpeg/webp/gif,无 svg/avif)
  sizeBytes: number;       // HeadObject 校验写入,≤ AVATAR_UPLOAD_MAX_BYTES(5MB)
  width: number;           // 像素,≤ AVATAR_TARGET_DIM(256)
  height: number;
  createdAt: bigint;       // Date.now() ms
}
```

**与 `attachments` 共用 MinIO 桶,但独立表** —— 不挂在 `pageId` 下,不参与 page cascade 语义。`GET /api/user-avatars/{userId}/raw` 公开 stream,跟 username 同级对外可见(`Cache-Control: private, max-age=300`)。`finalize` 后 `cleanupUserAvatars(userId)` 保留最新一行,删除其余 + best-effort S3。索引 1 条:`user_avatars_user_idx(userId, createdAt)`,「我的最新头像」hot path。

### `space_role_grants`(Phase A,空间角色授予)

```ts
{
  id: string;              // nanoid(10),与其他表 ID 同套
  spaceId: string;         // No FK
  principalKind: 'user' | 'group'; // 区分外部协作者直授 vs 整组授予
  principalId: string;     // No FK — users.id or user_groups.id
  role: 'viewer' | 'editor' | 'admin';
  grantedBy: string | null; // null = 系统操作(ensurePersonalSpace / bootstrap)
  grantedAt: bigint;
}
```

**UNIQUE `(space_id, principal_kind, principal_id)`** —— 同 principal 重复 grant idempotent,UPSERT。`role='admin'` 拒授给 group(后端 `assertNotAdminToGroup` 校验,仅 user 可拿 admin 权限)。**向后兼容**:`space_group_access` legacy 行在写入新 grant 时由 `migrateLegacyGroupGrant` 同事务迁入本表 `role='editor'`,老 API 仍保留作 rollback 安全网。

### `page_restrictions`(Phase B,页面级限制)

```ts
{
  id: string;
  spaceId: string;         // 冗余,审计 / 后台筛选用,跟 page_id 一起保留避免 LEFT JOIN
  pageId: string;          // No FK
  kind: 'view' | 'edit';   // 两个独立 allow-list,语义互不干扰
  principalKind: 'user' | 'group';
  principalId: string;
  grantedBy: string | null;
  grantedAt: bigint;
}
```

**反 default-deny**:某 page 出现 `kind='view'` 行 = 限制生效,非列表成员一律拒绝;**作者本人 + global admin 短路**(在 `canReadPage` / `canEditPage` 顶头返回,不走 allow-list)。**view 继承父链**(BFS 累加 allow-list),**edit 不继承**(只约束本页)。`view` 限制 + 公开分享互斥 —— POST share 时后端强校验,有 view 限制就 400 `share_forbidden`。索引 3 条:`page_restrictions_page_idx` + `page_restrictions_principal_idx(kind, principalKind, principalId)`(反查 my-role hot path)+ UNIQUE `(page_id, kind, principal_kind, principal_id)`。

### `page_public_shares`(Phase D,公开链接)

```ts
{
  id: string;
  pageId: string;          // No FK
  tokenHash: string;       // sha256(token) hex,**UNIQUE**,明文 token 仅 POST 响应里出现这一次
  createdBy: string;       // users.id,free-form
  createdAt: bigint;
  expiresAt: bigint | null; // null = 永不过期
  revokedAt: bigint | null;
  revokedBy: string | null;
  lastAccessedAt: bigint | null; // GET 命中时 fire-and-forget 更新,纯运营位
}
```

**index 2 条**:`page_public_shares_page_idx(pageId)`(「某 page 哪些 share 在跑」查询)+ `page_public_shares_expires_idx(expiresAt)`(lazy 清理 future 用)。匿名 GET `/api/public/pages/:token` 走 `tokenHash` 唯一索引精确查表(`UNIQUE` 在 tokenHash 列上),校验链:sha256 命中 + `revokedAt IS NULL` + (`expiresAt IS NULL OR > now`) + page `deletedAt IS NULL` + `spaces.kind='shared'`。任一失败 → 404。

### `permission_audit`(Phase C,append-only 审计日志)

```ts
{
  id: string;
  kind: 'space_grant_set' | 'space_grant_add' | 'space_grant_remove'
      | 'page_restriction_set' | 'page_restriction_add' | 'page_restriction_remove'
      | 'page_share_create' | 'page_share_revoke'
      | 'space_deleted' | 'group_deleted' | 'user_anonymized'; // 11 kind,CHECK 在 migration
  targetKind: 'space' | 'page' | 'page_share' | 'group' | 'user'; // 5 target_kind
  targetId: string;
  actorId: string;         // 操作人,No FK;actor 被 disable / delete 后 audit 行仍保留
  payload: jsonb | null;   // {before?, after?},kind 分支解析
  createdAt: bigint;
}
```

**严格 append-only** —— GET 是唯一入口,无 POST / PATCH / DELETE 暴露给 HTTP 层;**唯一写入路径** `recordPermissionAudit(tx, ...)` 必须包在 `db.transaction()` 内调用,**不**允许 transaction 外的孤行写入(否则审计行能预 commit / 滞后 commit,语义错位)。CHECK constraint 在 0029 迁移里把合法 `(kind, target_kind)` 对枚举出来,过期对会 INSERT 失败。

## Recursive CTE 模式

**没有 FK CASCADE**,所有层级删除 / 移动必须显式用 recursive CTE 一次拿全。

### 移动循环保护

`apps/api/src/lib/ids.ts` 的 `isDescendantOrSelf(parentId, candidateChildId)`:

```sql
WITH RECURSIVE subtree AS (
  SELECT id FROM pages WHERE id = $1
  UNION ALL
  SELECT p.id FROM pages p JOIN subtree s ON p.parent_id = s.id
)
SELECT 1 FROM subtree WHERE id = $2 LIMIT 1;
```

如果新父节点是当前节点的子孙 → 返 409 `cycle`。同文件还有 `getPageSubtree(rootId)` —— page purge 时返回整棵子树的 id 列表(配套 `pages.ts` DELETE 用)。

### 级联硬删

`pages.ts` DELETE `?purge=true` 走 multi-table recursive CTE,事务内清理 8 张页相关表(顺序重要,FK 反模式不存在所以**没有** FK 错误):

```sql
-- Step 1: 拿子树
WITH RECURSIVE subtree AS (
  SELECT id FROM pages WHERE id = $1
  UNION ALL
  SELECT p.id FROM pages p JOIN subtree s ON p.parent_id = s.id
)
-- 事务内 10 张 page-relation 表按顺序清
DELETE FROM notifications       WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM comments            WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM page_versions       WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM page_restrictions   WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM page_labels         WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM page_likes          WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM user_recent_pages   WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM attachments         WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM page_public_shares  WHERE page_id IN (SELECT id FROM subtree);
DELETE FROM pages               WHERE id       IN (SELECT id FROM subtree);
```

**故意不清的两张**(设计选择,不是漏):

- `user_watched_pages` —— watch 历史是用户级 metadata,删页不应抹掉用户「我曾 watch 过一个叫 X 的页」的痕迹(leave the row,eventual GC)
- `page_events` —— ActivityView 数据源,purge 事件本身仍记录(actor + spaceId + timestamp),read 端 LEFT JOIN 缺页时显示 `(无标题)` 而非丢行

S3 对象在事务外 best-effort `deleteObject`(失败仅日志,orphan 容忍)。

类似模式:
- `adminGroups.ts` DELETE → 事务里先扫 `userGroupMembers` + `spaceGroupAccess` 清理
- `adminSpaces.ts` DELETE → 事务里先扫 `spaceGroupAccess` + 同样五张 page-relation 表(无 pages 自分支因为 space 下可能有 page,但 page 自身有 spaceId,空间删除得先把 pages 移走或一起删)
- `adminUsers.ts` disable / reset-password → 先调 `killSessionsForUser` 清 sessions

### 可见空间过滤

`apps/api/src/lib/accessibleSpaceIds.ts` 根据 userId 查可见 space id 列表:

```sql
SELECT s.id
FROM spaces s
JOIN space_group_access sga ON sga.space_id = s.id
JOIN user_group_members ugm ON ugm.group_id = sga.group_id
WHERE ugm.user_id = $1
```

admin 自动看全部,所以 `routes/pages.ts` 的 list 走两路:admin 不 join,普通用户 join。`canAccessSpace(userId, spaceId)` 是单点 O(1) 检查(看 spaceId 是否在结果集里)。

## 软删除 + 回收站(全表统一模式)

**所有可软删的表都遵循 `deletedAt IS NULL` = 正常,`IS NOT NULL` = 软删** 这个语义:

- `pages.deletedAt` / `pages.deletedBy` —— 回收站(详见 [docs/architecture.md](./architecture.md))
- `comments.deletedAt` / `comments.deletedBy` —— author 或 admin 可软删;行保留让已发出的通知不丢锚点(UI 降级为 page-level 链接)

后端 list 已过滤,前端 `getTree()` 也再过一道 `p.deletedAt == null`(兜底,乐观更新 `softDeletePage` 在响应回来前会有 deletedAt != null 的行短暂存在)。

## Auth 设计

**自研薄 auth**(不引 third-party):

- 密码哈希:`@node-rs/argon2`(argon2id)
- Sessions:`sessions` 表 + HTTP-only cookie(`pw_session`, `SameSite=Lax`, 30 天固定过期)
- 启动时 `purgeExpiredSessions` 自动清理过期
- `auth/session.ts` 的 `killSessionsForUser(userId)` 在 admin 禁用 / 重置密码时清空该用户所有 session
- 三态登录:`active` / `disabled` / `must_reset_password`,新用户首次登录强制改密

中间件:
- `requireAuth` → 没 cookie 或 session 失效返 401
- `requireAdmin` → user.role !== 'admin' 返 403
- `assertAdminNotWritingPersonalSpace(c, me, targetSpaceId)` → 写路径(POST / PATCH / PATCH-move / DELETE-soft)的最后一道 guard

## Personal Space 自动初始化

`apps/api/src/lib/ensurePersonalSpace.ts`:

首次登录(以及 bootstrap 启动时给每个用户跑一遍):

1. 建 personal space(kind='personal')
2. 建 `pg-<userId>` 组(只有该用户一个成员)
3. 给 personal space 授权给该组
4. 复制一份欢迎页模板到该 space —— 标题 `欢迎来到 ${userName} 的个人空间`,内容是 Tiptap doc 树 + 匹配的 HTML(HTML 是 canonical 渲染,ReadView 走 v-html)

这一套在 `db/bootstrap.ts` 启动期 + `auth.ts` 路由的「首次 sign-in」两个时机各跑一遍。

## Admin 写保护

`apps/api/src/lib/personalSpaceGuard.ts` 的核心函数:

```ts
async function assertAdminNotWritingPersonalSpace(
  c: Context,
  me: AuthenticatedUser,
  targetSpaceId: string | null,
): Promise<Response | null>
```

- 3 个参数(Hono context + 当前用户 + 目标 spaceId)
- 非 admin / 没 targetSpaceId → 返 null(caller 继续)
- admin + `spaces.kind === 'personal'` → 返 403 `personal_space_readonly`(caller 直接 `return c.json(...)`)
- 同文件 `getSpaceKind(spaceId)` 拿 kind,O(1) 单行查

写路径在 `pages.ts` 的 POST / PATCH / PATCH-move / DELETE-soft 都调这个 guard。Confluence Cloud 默认语义:admin 是 supervisor 不是 editor。

## Index 策略

实际建好的索引(只列业务 hot path,PK 不计):

| 索引 | 用途 |
|---|---|
| `pages(parentId)` | 子页查询(树懒加载) |
| `pages(parentId, sortOrder)` | 同级排序(拖拽) |
| `pages(spaceId)` | 按 space 过滤 |
| `pages(spaceId, deletedAt)` | 回收站 partial-ish 过滤 |
| `page_versions(pageId, versionNumber)` | 历史 list(DESC) |
| `page_versions(pageId, versionNumber)` UNIQUE | 并发 PATCH 保护 |
| `page_labels(label)` | 标签搜索 hot path |
| `comments(pageId, createdAt)` | 评论 list |
| `comments(parentId)` | replies 关联 |
| `comments(authorId)` | 用户维度查询 |
| `comments(pageId, deletedAt)` | live 行过滤 |
| `notifications(userId, isRead, createdAt)` | bell 红点 hot path |
| `notifications(userId, createdAt)` | 通知 list |
| `notifications(pageId)` | page purge cascade |
| `attachments(pageId, createdAt)` | 附件 list |
| `attachments(uploaderId)` | 用户维度查询 |
| `page_likes(pageId)` | 某页点赞数 correlated subquery hot path |
| `page_likes(userId)` | 未来"我赞过哪些页面" |
| `page_events(spaceId, createdAt DESC)` | ActivityView 按 space 过滤 + 时间倒序 |
| `page_events(createdAt DESC)` | 全局 ActivityView 列表(无 space 过滤) |
| `user_watched_pages(userId)` | `/api/users/me/watched` + Sidebar/Dashboard 排序 |
| `user_watched_pages(pageId)` | 「该页多少人关注」`COUNT(*)` |
| `user_group_members(userId, groupId)` | 复合 PK |
| `space_group_access(spaceId, groupId)` | 复合 PK |
| `sessions(userId)` | kill-sessions |
| `user_avatars(userId, createdAt)` | 「我的最新头像」finalize 后 cleanup 路径 + 头像历史查询 |
| `space_role_grants(spaceId)` | 空间设置页拉该空间所有 grants |
| `space_role_grants(principalKind, principalId)` | 反查「我作为 user/group 在哪些 space 有何角色」(permission SQL 核心 join) |
| `space_role_grants(spaceId, principalKind, principalId)` UNIQUE | 幂等 UPSERT 唯一约束 |
| `page_restrictions(pageId)` | 检查某 page 是否存在 view / edit 限制 |
| `page_restrictions(kind, principalKind, principalId)` | 反查「我的用户在哪些受限页里有 view / edit」热路径 |
| `page_restrictions(pageId, kind, principalKind, principalId)` UNIQUE | 幂等 UPSERT 唯一约束 |
| `page_public_shares(pageId)` | 「某 page 哪些 share 在跑」列表 |
| `page_public_shares(expiresAt)` | 过期 lazy 清理路径 |
| `page_public_shares(tokenHash)` UNIQUE | 匿名 GET `/api/public/pages/:token` 精确查 hot path(列上 UNIQUE 约束) |
| `permission_audit(targetKind, targetId, createdAt DESC)` | 后台 audit 详情页:某 target 全部事件按时间倒序 |
| `permission_audit(actorId, createdAt DESC)` | 后台 audit 详情页:某 actor 全部事件 |
| `permission_audit(createdAt DESC)` | audit 全局列表 hot path |

`admin_settings` 是 PK 即 `key`,无额外索引。

具体 DDL 走 `pnpm -F api db:generate` 生成迁移,不要手写。新增索引必须同步 `apps/api/src/db/schema.ts` 的 JSDoc **和** 迁移里的 `COMMENT ON INDEX`(见 [CLAUDE.md 硬约束](../CLAUDE.md))。

## Schema drift 修正记录

CLAUDE.md 硬约束第 10 条:schema JSDoc 与 SQL `COMMENT` 必须同步,drift 视为 bug。

| migration | 修正项 | 原因 |
|---|---|---|
| `0011_fix_schema_drift.sql` | `users.password_hash` SQL COMMENT 改回 argon2id | 0008 误写 bcrypt,实际用 `@node-rs/argon2`(见 `apps/api/src/auth/password.ts`),下一个维护者按 SQL COMMENT 改 bcrypt 验证会破坏所有登录 |
| `0011_fix_schema_drift.sql` | `notifications.kind` SQL COMMENT 补 `page_like` | schema.ts enum 已含 4 种(0010 加 page_like),0008 SQL 注释落后 |
| `0011_fix_schema_drift.sql` | 新建 `page_likes_page_idx(pageId)` | schema.ts JSDoc 提到该索引但 0010 没建;`selectPagesWithAuthor` 的 `likesCount` correlated subquery 现在走全表,加索引后 COUNT(*) 走 index-only scan |

**注**:0011 是纯 SQL 迁移(COMMENT / INDEX),drizzle-kit 对这类迁移会**静默跳过**——日志报 "applied" 但数据库什么也没改。验证必须走 psql 直接看 `pg_description` 和 `\d page_likes`,不能信 drizzle 日志。

未来新表 / 新列默认带 COMMENT,新加索引必须同步 schema.ts JSDoc + 迁移 `COMMENT ON INDEX`,drizzle 静默跳过需要 psql 兜底验证。
